...
from .layers import MultGraphAttentionLayer
...
class MultGAT(nn.Module):
    def __init__(self, hpars):
        super(MultGAT, self).__init__()
        self.hpars = hpars
        ...
        for i in range(self.model_depth):
            if i==0:
                ...
                self.attentions = [MultGraphAttentionLayer(hpars=self.hpars,
                ...
                self.attentions = [
                    MultGraphAttentionLayer(hpars=self.hpars,
                                        in_feats= self.hidden_dim,
                                        out_feats= int(self.hidden_dim / self.n_heads),
                                        edge_feat_dim = self.edge_feats_dim,
                                        final=False) for _ in range(self.n_heads)]
